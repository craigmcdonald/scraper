  ...
  module Data
    class Model
      ...
      def store_data(data)
        @data = Store[
          data.map do |k,v|
            [k, v .is_a?(Array) ? Collection.new({k => v}) : v]
          end
        ]
      end
      ...
      def upsert_with_collection(key,*args)
        upsert(key, Collection.new({key => args}))
      end
      ...
      def key_level(key)
        ...
        hash[key] || -> { raise Errors::KeyNotFound.new }.call
      end
